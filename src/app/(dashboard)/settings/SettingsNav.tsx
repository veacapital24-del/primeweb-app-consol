...
const SECTIONS = [
  ...
  { href: '/settings/team',         label: 'Team & roles', desc: 'Admin, wholesaler, retailer' },
  { href: '/settings/customers',    label: 'Customers',    desc: 'End-shopper accounts' },
            ...
            return (
              <Link
                ...
                className={`flex flex-col gap-0.5 rounded-xl px-3 py-2.5 transition ${
                  active
                    ? 'bg-prime-50 ring-1 ring-prime-200'
                    : 'hover:bg-paper-dim/60'
                }`}
              >
                <span className={`text-sm font-bold ${active ? 'text-prime-700' : 'text-ink-900'}`}>
                  {s.label}
                </span>
                <span className="text-[11px] text-ink-500">{s.desc}</span>
              </Link>
            )
          })}
        </div>
            ...
            return (
              <Link
                ...
                className={`shrink-0 rounded-full px-3 py-1.5 transition ${
                  active ? 'bg-ink-900 text-paper' : 'bg-paper text-ink-700 ring-1 ring-ink-200'
                }`}
              >
                {s.label}
              </Link>
